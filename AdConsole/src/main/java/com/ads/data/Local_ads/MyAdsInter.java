...
public class MyAdsInter {
    ...
    public MyAdsInter(final Activity act, final MyAd myAd, OnClickListener myCallback2) {
        ...
        if (app_data != null && app_data.size() > 0) {
            ...
            try {
                Glide.with(act).load(myAd.getApp_icon()).into(appicon);
                Glide.with(act).load(myAd.getApp_banner()).into(ad_banner);
            ...
            install.setOnClickListener(v -> {
                if (myAd.getPakag().equals(app_data.get(0).getQureka_url())) {
                    try {
                        ...
                        Conts.openCustomTab(act, customIntent.build(), Uri.parse(myAd.getPakag()));
                    } catch (Exception e) {
